...
import static ru.practicum.statsvc.model.dto.EventMapper.eventFromDto;
...
public class EventServiceImpl implements EventService {
    ...
    public List<EventDtoOut> getEvents(LocalDateTime start, LocalDateTime end, List<String> uris, boolean unique) {
        ...
        if (uris == null || uris.isEmpty()) {
            if (unique) return eventStorage.getAllStatsWithUniqueIps(start, end);
            return eventStorage.getAllStats(start, end);
        } else {
            if (unique) return eventStorage.getAllStatsWithUrisAndUniqueIps(start, end, uris);
            return eventStorage.getAllStatsWithUris(start, end, uris);
        }
    }
}